#!/usr/bin/env python3
"""
Script final para corrigir as trilhas restantes
"""
...
def limpar_nome_complexo(nome_trilha):
    """
    Limpa nomes complexos e duplicados
    """
    if pd.isna(nome_trilha) or not nome_trilha:
        return nome_trilha
    ...
    # Se o nome é muito longo, pegar apenas a primeira parte
    if len(nome) > 200:
        # Dividir por " - " e pegar apenas as primeiras partes
        partes = nome.split(' - ')
        if len(partes) > 3:
            # Pegar apenas as primeiras 3 partes
            nome_limpo = ' - '.join(partes[:3])
            return nome_limpo
    
    return nome

def corrigir_trilhas_finais():
    """
    Corrige as trilhas finais que ainda estão sem código
    """
    print("🔧 Correção final das trilhas restantes")
    print("="*50)
    ...
    if len(trilhas_sem_codigo) > 0:
        ...
        for i, item in enumerate(trilhas_sem_codigo[:3], 1):
            print(f"  {i}. {item['trilha'][:100]}...")
        
        # Aplicar códigos baseados em correspondências específicas
        correcoes = []
        cursor = conn2.cursor()
        ...
        for item in trilhas_sem_codigo:
            ...
            # Limpar o nome complexo
            trilha_limpa = limpar_nome_complexo(trilha)
            
            # Buscar correspondência na gestão
            codigo_encontrado = None
            
            # Primeiro, tentar correspondência exata com nome limpo
            ...
            if not match_gestao.empty:
                ...
                if pd.notnull(codigo_gestao) and codigo_gestao and codigo_gestao != 'nan':
                    codigo_encontrado = codigo_gestao
            
            # Se não encontrou, tentar correspondência parcial
            if not codigo_encontrado:
                # Buscar por "SD Fertilizantes - Compra Direta + Venda com Troca De Notas"
                if "SD Fertilizantes - Compra Direta + Venda com Troca De Notas" in trilha:
                    # Buscar na gestão por trilhas que contenham essa frase
                    for _, row_gestao in df_gestao.iterrows():
                        ...
                        if (pd.notnull(codigo_gestao) and codigo_gestao and codigo_gestao != 'nan' and
                            "SD Fertilizantes - Compra Direta + Venda com Troca De Notas" in trilha_gestao):
                            codigo_encontrado = codigo_gestao
                            break
            
            # Se encontrou código, aplicar correção
            if codigo_encontrado:
                novo_nome = f"{codigo_encontrado} - {trilha_limpa}"
                correcoes.append({
                    'id': item['id'],
                    'trilha': trilha,
    ...
    if trilhas_sem_codigo_final:
        ...
        for i, trilha in enumerate(trilhas_sem_codigo_final[:3], 1):
            print(f"  {i}. {trilha[:100]}...")
    
    conn2.close()
    conn_gestao.close()

if __name__ == "__main__":
    corrigir_trilhas_finais() 