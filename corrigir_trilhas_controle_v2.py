#!/usr/bin/env python3
"""
Script para corrigir trilhas sem código na tabela controle_trilhas (Versão 2)
Adiciona códigos extraídos dos nomes das trilhas sem duplicação
"""
...
def corrigir_trilhas_controle():
    ...
    """
    print("🔧 Corrigindo trilhas na tabela controle_trilhas (V2)")
    print("="*60)
    ...
    if len(trilhas_sem_codigo) > 0:
        ...
        for item in trilhas_sem_codigo:
            ...
            if not match_gestao.empty:
                ...
                # Buscar por correspondência parcial (sem o código no início)
                for _, row_gestao in df_gestao.iterrows():
                    trilha_gestao = row_gestao['Trilhas']
                    codigo_gestao = row_gestao['Código']
                    
                    # Se a trilha da gestão tem código e corresponde à trilha do controle
                    if (pd.notnull(codigo_gestao) and codigo_gestao and codigo_gestao != 'nan' and
                        trilha_gestao.endswith(trilha)):
                        correcoes.append({
                            'id': item['id'],
                            'trilha': trilha,
                            'codigo': codigo_gestao,
                            'fonte': 'gestao_parcial'
                        })
                        break
        ...
        if len(correcoes) > 0:
            ...
            for correcao in correcoes:
                ...
                print(f"  ✅ {correcao['trilha'][:50]}... → {correcao['codigo']} - {correcao['trilha'][:30]}... ({correcao['fonte']})")
    ...
    if trilhas_sem_codigo_final:
        print(f"\n⚠️ Exemplos de trilhas sem código:")
...
def limpar_duplicatas():
    """
    Remove duplicatas na tabela controle_trilhas
    """
    print("\n🧹 Removendo duplicatas na tabela controle_trilhas")
    print("="*55)
    ...
    if len(duplicatas) > 0:
        ...
        for i, (_, row) in enumerate(duplicatas.head(3).iterrows(), 1):
            print(f"  {i}. {row['Trilhas'][:50]}...")
